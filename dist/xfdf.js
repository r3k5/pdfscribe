...
export function toXfdf(annotations, pdfFilename = '') {
...
function parseCommon(el) {
    ...
    return {
        ...
        intent: gattr(el, 'intent') || undefined,
        rect: rect ? parseFloats(rect) : [0, 0, 0, 0],
    };
}
function parseOne(el) {
    ...
    switch (tag) {
        case 'ink': {
            ...
            return { ...base, subtype: 'Ink', inkList: gestures };
        }
        case 'freetext': {
            const cl = gattr(el, 'callout-line');
            const rc = el.querySelector('body')?.textContent ?? base.contents ?? '';
            return { ...base, subtype: 'FreeText', calloutLine: cl ? parseFloats(cl) : undefined, richText: rc, defaultStyle: gattr(el, 'defaultappearance') || undefined };
        }
        case 'line': {
            const start = parseFloats(gattr(el, 'start'));
            const end = parseFloats(gattr(el, 'end'));
            return { ...base, subtype: 'Line', line: [start[0], start[1], end[0], end[1]], lineEndings: [gattr(el, 'head') || 'None', gattr(el, 'tail') || 'None'] };
        }
        case 'polyline':
            return { ...base, subtype: 'PolyLine', vertices: parseFloats(gattr(el, 'vertices')) };
        case 'polygon':
            return { ...base, subtype: 'Polygon', vertices: parseFloats(gattr(el, 'vertices')), cloudEffect: gattr(el, 'cloudy') === '1' };
        case 'circle':
            return { ...base, subtype: 'Circle' };
        case 'square':
            return { ...base, subtype: 'Square' };
        case 'highlight': {
            const coords = gattr(el, 'coords');
            return { ...base, subtype: 'Highlight', quadPoints: coords ? parseFloats(coords) : undefined };
        }
        default:
            return null;
    }
...
export function fromXfdf(xfdf) {
    const doc = new DOMParser().parseFromString(xfdf, 'application/xml');
    const annots = doc.querySelector('annots');
    if (!annots)
        return [];
    return Array.from(annots.children).map(parseOne).filter((a) => a !== null);
}
//# sourceMappingURL=xfdf.js.map