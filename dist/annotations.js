...
function buildFontResources(ctx) {
    ...
    return ctx.obj({ Font: ctx.obj({ Helv: ctx.register(fontDict) }) });
}
function buildAppearanceStream(ctx, ann) {
    ...
    switch (ann.subtype) {
        case 'Ink': {
            for (const stroke of ann.inkList ?? []) {
                ...
                for (let i = 2; i < stroke.length - 2; i += 2)
                    push(`${f(stroke[i])} ${f(stroke[i + 1])} l`);
                push('S');
            }
            break;
        }
        ...
        case 'Highlight': {
            ...
            const gsRef = ctx.register(ctx.obj({ Type: PDFName.of('ExtGState'), ca: PDFNumber.of(opacity), CA: PDFNumber.of(opacity), BM: PDFName.of('Multiply') }));
            const pad = sw + 2;
...
export function buildAnnotDict(pdfDoc, pageRef, ann, creatorTool) {
    ...
    if (ann.contents)
        dict.set(PDFName.of('Contents'), PDFString.of(ann.contents));
    if (ann.opacity !== undefined && ann.opacity < 1)
        dict.set(PDFName.of('CA'), PDFNumber.of(ann.opacity));
    if (ann.intent)
        dict.set(PDFName.of('IT'), PDFName.of(ann.intent));
    if (ann.cloudEffect) {
    ...
    switch (ann.subtype) {
        ...
        case 'FreeText': {
            ...
            if (ann.richText)
                dict.set(PDFName.of('RC'), PDFString.of(ann.richText));
            if (ann.defaultStyle)
                dict.set(PDFName.of('DS'), PDFString.of(ann.defaultStyle));
            if (ann.calloutLine && ann.calloutLine.length >= 4) {
                const cl = PDFArray.withContext(ctx);
                for (const n of ann.calloutLine)
                    cl.push(PDFNumber.of(n));
        ...
        case 'Line': {
            if (ann.line)
                dict.set(PDFName.of('L'), ctx.obj(ann.line.map(PDFNumber.of)));
            if (ann.lineEndings)
                dict.set(PDFName.of('LE'), ctx.obj(ann.lineEndings.map(PDFName.of)));
        ...
        case 'Polygon': {
            if (ann.vertices) {
                ...
                for (const n of ann.vertices)
                    verts.push(PDFNumber.of(n));
        ...
        case 'Highlight': {
            if (ann.quadPoints) {
                ...
                for (const n of ann.quadPoints)
                    qp.push(PDFNumber.of(n));
    ...
    if (apRef)
        dict.set(PDFName.of('AP'), ctx.obj({ N: apRef }));
    return ctx.register(dict);
}
/** Embed annotations into a PDF page. Resolves /Annots as direct array or indirect ref. */
export async function embedAnnotations(pdfDoc, annotations, pageIndex = 0, creatorTool = '@r3k5/pdfscribe') {
    const ctx = pdfDoc.context;
